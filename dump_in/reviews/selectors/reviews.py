...
class ReviewSelector:
    def get_review_with_concept_queryset_by_photo_booth_id_order_by_created_at_desc(self, photo_booth_id: str) -> QuerySet[Review]:
        return Review.objects.prefetch_related("concept").filter(photo_booth_id=photo_booth_id, is_public=True).order_by("-created_at")
    ...
    def get_review_with_concept_queryset_by_photo_booth_brand_id_order_by_created_at_desc(
        ...
        return (
            ...
            .filter(photo_booth__photo_booth_brand_id=photo_booth_brand_id, is_public=True)
            .order_by("-created_at")
        )

    def get_review_by_id_and_user_id(self, review_id: int, user_id) -> Optional[Review]:
        try:
            return Review.objects.filter(id=review_id, user_id=user_id).get()
    ...
    def get_public_review_by_id(self, review_id: int) -> Optional[Review]:
        try:
            return Review.objects.filter(id=review_id, is_public=True).get()
        except Review.DoesNotExist:
            return None

    def get_review_with_photo_booth_and_brand_queryset_by_user_id(self, user_id) -> QuerySet[Review]:
        return Review.objects.select_related("photo_booth").prefetch_related("photo_booth__photo_booth_brand").filter(user_id=user_id)

    def get_review_with_photo_booth_queryset_by_user_like(self, user_id) -> QuerySet[Review]:
        return Review.objects.select_related("photo_booth").filter(userreviewlikelog__user_id=user_id, is_public=True)
