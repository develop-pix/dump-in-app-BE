...
class TestReviewService:
    ...
    def test_create_review_success(self, new_users, photo_booth, hashtag):
        ...
        assert review.hashtags.all().count() == 1
    ...
    def test_update_review_success(self, photo_booth, hashtag, review):
    ...
    def test_update_review_fail_image_urls_list_many(self, photo_booth, hashtag, review):
        with pytest.raises(ValidationException) as e:
            review = self.review_service.update_review(
                review_id=review.id,
                image_urls=["test1", "test2", "test3", "test4", "test5", "test6"],
    ...
    def test_update_review_fail_photo_booth_does_not_exist(self, hashtag, review):
        with pytest.raises(NotFoundException) as e:
            review = self.review_service.update_review(
                review_id=review.id,
                image_urls=["test1", "test2", "test3"],
    ...
    def test_update_review_fail_review_does_not_exist(self, photo_booth, review, hashtag):
        with pytest.raises(NotFoundException) as e:
            review = self.review_service.update_review(
                review_id=100,
                image_urls=["test1", "test2", "test3"],
    ...
    def test_update_review_fail_user_permission_denied(self, photo_booth, hashtag, review, new_users):
