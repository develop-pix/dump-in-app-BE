...
class TestViewCountUp:
    ...
    def test_view_count_up_success(self, valid_review):
        ...
        review = self.review_service.view_count_up(review_id=valid_review.id, user_id=valid_review.user.id)

        assert review.view_count == before_view_count + 1

    def test_view_count_up_fail_review_does_not_exist(self):
        with pytest.raises(NotFoundException) as e:
            self.review_service.view_count_up(review_id=999, user_id=None)
    ...
    def test_view_count_up_fail_permission_denied(self, private_review, valid_user):
        with pytest.raises(PermissionDeniedException) as e:
            self.review_service.view_count_up(review_id=private_review.id, user_id=valid_user.id)
