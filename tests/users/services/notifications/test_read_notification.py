...
class TestReadNotification:
    ...
    def test_read_notification_success(self, valid_notification):
        notification = self.notification_service.read_notification(
            notification_id=valid_notification.id, user_id=valid_notification.user_id
        )
    ...
    def test_read_notification_fail_does_not_exist(self, valid_user):
        with pytest.raises(NotFoundException) as e:
            self.notification_service.read_notification(notification_id=999, user_id=valid_user.id)
    ...
    def test_read_notification_fail_permission_denied(self, valid_notification):
        with pytest.raises(PermissionDeniedException) as e:
            self.notification_service.read_notification(notification_id=valid_notification.id, user_id=999)
