...
class TestGetEventQuerysetByPhotoBoothBrandId:
    ...
    def test_get_event_queryset_by_photo_booth_brand_id_success_single_event(self, valid_event, valid_user):
        event_queryset = self.event_selector.get_event_queryset_by_photo_booth_brand_id(valid_event.photo_booth_brand_id, valid_user.id)

        assert event_queryset.first() == valid_event

    def test_get_event_queryset_by_photo_booth_brand_id_success_multiple_event(self, valid_event_list, valid_user):
        event_queryset = self.event_selector.get_event_queryset_by_photo_booth_brand_id(
            valid_event_list[0].photo_booth_brand_id, valid_user.id
        )
    ...
    def test_get_event_queryset_by_photo_booth_brand_id_fail_private_event(self, private_event, valid_user):
        event_queryset = self.event_selector.get_event_queryset_by_photo_booth_brand_id(private_event.photo_booth_brand_id, valid_user.id)

        assert event_queryset.count() == 0

    def test_get_event_queryset_by_photo_booth_brand_id_fail_not_event_photo_booth_brand(self, invalid_event, valid_user):
        event_queryset = self.event_selector.get_event_queryset_by_photo_booth_brand_id(invalid_event.photo_booth_brand_id, valid_user.id)
