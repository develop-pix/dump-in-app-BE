...
class TestGetEventWithUserInfoById:
    ...
    def test_get_event_with_user_info_by_id_success(self, valid_event, valid_user):
        event = self.event_selector.get_event_with_user_info_by_id(valid_event.id, valid_user)
    ...
    def test_get_evnet_with_user_info_by_id_success_anonymous_user(self, valid_event, inactive_user):
        event = self.event_selector.get_event_with_user_info_by_id(valid_event.id, inactive_user)

        assert event == valid_event

    def test_get_event_with_user_info_by_id_fail_does_not_exist(self, valid_event, valid_user):
        event = self.event_selector.get_event_with_user_info_by_id(valid_event.id + 1, valid_user)

        assert event is None

    def test_get_event_with_user_info_by_id_fail_private_event(self, private_event, valid_user):
        event = self.event_selector.get_event_with_user_info_by_id(private_event.id, valid_user)

        assert event is None

    def test_get_event_with_user_info_by_id_fail_not_event_photo_booth_brand(self, invalid_event, valid_user):
        event = self.event_selector.get_event_with_user_info_by_id(invalid_event.id, valid_user)
